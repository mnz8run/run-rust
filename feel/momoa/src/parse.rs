...
impl<'a> Parser<'a> {
    ...
    fn peek_token(&mut self) -> Option<Result<Token, MomoaError>> {
        ...
        // println!("peek_token {:?}", self.peeked);
        if let Some(token) = self.peeked {
            return Some(Ok(token));
        }
    ...
    fn parse_boolean(&mut self) -> Result<Node, MomoaError> {
        ...
        println!("parse_boolean {:?}", token);
        let text = self.get_text(token.loc.start.offset, token.loc.end.offset);
        let value = text == "true";

        return Ok(Node::Boolean(Box::new(ValueNode { value, loc: token.loc })));
    }
    ...
    fn parse_number(&mut self) -> Result<Node, MomoaError> {
        ...
        return Ok(Node::Number(Box::new(ValueNode { value, loc: token.loc })));
    }
    ...
    fn parse_string(&mut self) -> Result<Node, MomoaError> {
        ...
        return Ok(Node::String(Box::new(ValueNode { value, loc: token.loc })));
    }
